...
def process_lobbying_report(row: Dict, db):
    ...
    proposed_start_date = (
        datetime.strptime(row["ProposedStartDate"], "%Y-%m-%d").date()
        if row.get("ProposedStartDate", None) is not None
        else None
    )
    proposed_end_date = (
        datetime.strptime(row["ProposedEndDate"], "%Y-%m-%d").date()
        if row.get("ProposedEndDate", None) is not None
        else None
    )
    initial_approval_date = datetime.strptime(
        row["InitialApprovalDate"], "%Y-%m-%d"
    ).date()
    effective_date = datetime.strptime(row["EffectiveDate"], "%Y-%m-%d").date()
    registrant = Registrant.query.filter_by(
        registration_number=row["Registrant"]["RegistrationNUmberWithSoNum"]
    ).one_or_none()
...
def process_prefix(raw_prefix: str) -> PersonPrefix:
    if raw_prefix is None or raw_prefix in [
        "M",
        "M.",
        "Aubrey Dan",
        "matt kosoy",
...
def process_registrant_old(registrant_dict, db):
    ...
    registration_number_with_senior_officer_number = registrant_dict[
        "RegistrationNUmberWithSoNum"
    ]
    status = RegistrantStatus(registrant_dict["Status"])
    effective_date = (
        datetime.strptime(registrant_dict["EffectiveDate"], "%Y-%m-%d").date()
        if registrant_dict.get("EffectiveDate", None) is not None
        else None
    )
    ...
    person = process_person(
        prefix, first_name, middle_initial, last_name, suffix, position_title
    )
...
def process_registrants(data_registrants: list[Data], db):
    for data_registrant in data_registrants:
        ...
        raw_registrant = TempRawRegistrant(
            ...
            RegistrationNUmberWithSoNum=raw_registrant_dict[
                "RegistrationNUmberWithSoNum"
            ],
            ...
            PreviousPublicOfficeHolder=raw_registrant_dict[
                "PreviousPublicOfficeHolder"
            ],
            PreviousPublicOfficeHoldPosition=raw_registrant_dict[
                "PreviousPublicOfficeHoldPosition"
            ],
            PreviousPublicOfficePositionProgramName=raw_registrant_dict[
                "PreviousPublicOfficePositionProgramName"
            ],
            PreviousPublicOfficeHoldLastDate=raw_registrant_dict[
        ...
        if (
            raw_registrant.PreviousPublicOfficeHolder == "No"
            and raw_registrant.PreviousPublicOfficeHoldPosition is not None
        ):
            raw_registrant.PreviousPublicOfficeHolder = "Yes"

        if raw_registrant.RegistrationNUmberWithSoNum == "33003C":
            raw_registrant.FirstName = (
                "Bradley"  # Sometimes FirstName == Brad Nickname?
            )
        if raw_registrant.RegistrationNUmberWithSoNum == "29745S-1":
            raw_registrant.LastName = "Fatehi"  # Sometimes LastName == Fatehi Somee IDK
        if raw_registrant.RegistrationNUmberWithSoNum == "17769C":
            raw_registrant.LastName = (
                "Tomasella"  # Sometimes LastName == Loiacono Married?
            )
        if raw_registrant.RegistrationNUmberWithSoNum == "19772C":
            raw_registrant.MiddleInitials = "C"  # Once MiddleInitials == M Typo?
        if raw_registrant.RegistrationNUmberWithSoNum == "18590C":
            raw_registrant.LastName = "Bassani"  # Sometimes LastName == Chien Married?
        if raw_registrant.RegistrationNUmberWithSoNum == "12126C":
            raw_registrant.FirstName = (
                "Leslie"  # Sometimes FirstName == Leslie M  Typo?
            )

        if (
            raw_registrant.RegistrationNUmberWithSoNum == "14409C"
        ):  # Sometimes PreviousPublicOfficeHolder == 'No'
            raw_registrant.PreviousPublicOfficeHolder = "Yes"
            raw_registrant.PreviousPublicOfficeHoldPosition = "Commissioner"
            raw_registrant.PreviousPublicOfficePositionProgramName = (
    ...
    registrants_with_prefix = (
        db.session.query(TempRawRegistrant.RegistrationNUmberWithSoNum)
        .filter(TempRawRegistrant.Prefix.isnot(None))
        .distinct()
        .all()
    )
    ...
    for registrant in registrants_with_prefix:
        non_null_prefixes = (
            db.session.query(TempRawRegistrant.Prefix)
            .filter(
                TempRawRegistrant.RegistrationNUmberWithSoNum
                == registrant.RegistrationNUmberWithSoNum,
                TempRawRegistrant.Prefix.isnot(None),
            )
            .distinct()
            .all()
        )
        ...
        if len(non_null_prefixes) > 1:
            raise ValueError(
                f"Multiple non-null prefixes found for RegistrationNUmberWithSoNum: {registrant.RegistrationNUmberWithSoNum}"
            )

        elif non_null_prefixes:
            db.session.query(TempRawRegistrant).filter(
                TempRawRegistrant.RegistrationNUmberWithSoNum
                == registrant.RegistrationNUmberWithSoNum
            ).update({TempRawRegistrant.Prefix: non_null_prefixes[0][0]})
    db.session.commit()
...
def get_non_superseded_registrants(rows: List[Dict]):
    return [
        row["Registrant"] for row in rows if row["Registrant"]["Status"] != "Superseded"
    ]
...
def get_data_registrants(data_rows: List[Data]) -> List[Data]:
    ...
    for data_row in data_rows:
        data_registrants.append(
            Data(data_value=data_row.data_value["Registrant"], source=data_row.source)
        )
