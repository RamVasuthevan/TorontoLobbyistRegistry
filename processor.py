...
from app.models import LobbyingReport, LobbyingReportStatus, LobbyingReportType, Registrant, RegistrantStatus, RegistrantType, get_enum_error_message, PersonPrefix, Person
...
def process_person(prefix, first_name, middle_initial, last_name, suffix):
...
def process_registrant(registrant_dict,db):
    ...
    person = process_person(prefix,first_name,middle_initial,last_name,suffix)

    if person is None:
        ValueError(registrant_dict)

    try:
        registrant = Registrant(registration_number=registration_number,registration_number_with_senior_officer_number=registration_number_with_senior_officer_number, status=status,effective_date=effective_date,type=_type,person=person)
    except Exception as e:
