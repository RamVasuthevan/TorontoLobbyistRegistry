...
def get_data_row(raw_beneficiary: RawBeneficiary, address_lookup: dict[int,int]) -> dict:
    return {
...
def create_beneficiaries_table(session: Session, raw_beneficiaries: List[RawBeneficiary]) -> List[Beneficiary]:
    ...
    data = [get_data_row(raw_beneficiary, address_lookup) for raw_beneficiary in raw_beneficiaries]
